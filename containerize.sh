...
docker run --rm -p 2181:2181 -d -e ZOOKEEPER_CLIENT_PORT=2181 --network app-kafka --name zookeeper-cntr zookeeper:img

docker build -t kafka:img -f .\Dockerfile.kafka . 
docker run --rm -p 9092:9092 -p 29092:29092 -e KAFKA_ZOOKEEPER_CONNECT=zookeeper-cntr:2181 -e KAFKA_ADVERTISED_LISTENERS=CLIENT://kafka-cntr:9092,EXTERNAL://localhost:29092 -d --network app-kafka --name kafka-cntr kafka:img 
...
docker run --rm --name backend-cntr -e SERVER_PORT=3000 -e MYSQL_USERNAME=root -e MYSQL_PASSWORD=Passw@rd -e DATABASE_URL=mysqldb -e DATABASE_PORT=3307 -e KAFKA_URL=kafka-cntr:9092 -p 3000:3000 -d --network app-api backend:v1
